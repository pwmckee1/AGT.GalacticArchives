...
{
    public override Guid EntityId => BaseId;

    public Guid BaseId { get; set; } = Guid.NewGuid();

    [Display(ResourceType = typeof(BaseResource), Description = nameof(BaseResource.Name))]
    ...
    {
        properties = typeof(Base).GetProperties(BindingFlags.Public | BindingFlags.Instance);
        excludedProperties =
        [
            nameof(EntityId),
            nameof(Planet),
